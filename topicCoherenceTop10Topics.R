...
source('functions/important_topics.R')
...
for(j in 1:length(topicsizes)){
  ...
  highest_top <- important_topics(topic.model, 10)
  
  #calculate top words
  topic.words <- mallet.topic.words(topic.model)
  df.words <- tibble()
  for(i in highest_top){
...
save(coherence_values, file = "rfiles/coherence_top10_5_100.rdata")
