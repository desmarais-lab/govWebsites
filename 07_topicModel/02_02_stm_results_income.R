...
for(j in 1:numtopics){
  ...
  for(i in 1:sims){
    coefs[[i]] <- prep$parameters[[j]][[i]]$est[9]
  }
...
blues <- rev(c("teal!10", "teal!20", "teal!30", "teal!40", "teal!50", "teal!60", "teal!70", "teal!80"))
reds <- c("orange!10", "orange!20", "orange!30", "orange!40", "orange!50", "orange!60", "orange!70", "orange!80")
...
xtTopwords <- print(xtable(topwords3, caption = "Top words from a structural topic model with 60 topics and FREX scoring. Colors depict city median income based on coefficient size (wealthier cities are orange, poorer cities are teal). White cells are non-significant topics.",
...
writeLines(xtTopwords, con = '../paper/tables/stmTopWords60_income.tex')
