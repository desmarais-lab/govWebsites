...
Consequently the covariates include:
\begin{itemize}
\item a categorical independent variable, indicating which topic a document gets assigned to under the expressed agenda model
%\item a variable indicating the change of party control at time $t$
\item is the party that put the document up still present at time $t$
\item interaction term between the two above
\end{itemize}
This will take up a lot of degrees of freedom.

Proportional hazard? If not, then likely rising, although there might be some documents that really do hardly ever get changed, so it should probably fall again after a few years.

Model document being changed vs. being removed as competing risks.
