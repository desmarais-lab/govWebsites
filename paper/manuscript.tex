...
An alternative approach to the problem is to ignore topics entirely, and go straight to predicting documents that are much more likely to be included on websites belonging to one or the other party. Classic machine learning techniques such as Naive Bayes, Linear Discriminant Analysis, or SVM should be expected to fare well in this context. Here, we rely on SVM, through three different implementations. The SciKitLearn package in Python, the e1071 package as well as the mlr package (relying on the kernlab package) in R. Predicting based on the document-term matrix (either in its regular form, or as its tf-idf representation), and using five-fold cros-validation, SciKitLearn produces a predictive accuracy of at most 78\%. Parameter tuning did not lead to any improvements. The e1071 package in R, relying on the top 5000 features ranked through tf-idf fared slightly better, at 82 percent correctly predicted. Finally, the mlr/kernlab implementation (as well as the e1071 implementation through mlr) has not worked at all so far, resulting in stack overflow errors (which appears to be due to memory limitations in R).

Another point worth noting is that \cite{Monroe2008} advise against using these types of methods in this context because they get the data generation process backwards: Our theory assumes that party leads to variation in writing, and yet we rely on the documents to predict party, in spite of the fact that we actually have perfect knowledge of it.

Other methods used: Elastic-net in the glmnet package in R. Accuracy is 0.6924795 for in-sample prediction, so not worth bothering with.
...
\begin{figure}[!ht]
	\centering % Using \begin{figure*} makes the figure take up the entire width of the page
	\caption{Top democratic and Republican words, according to the informed Dirichlet model of Monroe et al. (2008).}
	\label{fightinwords}
	\includegraphics[width=\linewidth]{figures/fightinWords.png}
\end{figure}

%Partisan top words
\input{tables/partisanTopWords.tex}

%Some basic descriptive statistics of documents by party
\input{tables/descriptiveStatisticsPartisan.tex}
