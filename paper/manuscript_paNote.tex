...
After all the preprocessing is set and done, our corpus consists of 259,099 documents.  In Table \ref{tab:list} we summarize all of the steps we take in gathering and processing our data. The summary includes a brief description of the step, the software packages used, and an indicator of whether the method is implemented in our R package, \texttt{gov2text}. 

\begin{table}[ht]
    \centering
    \begin{tabular}{lrr}
        \hline
        Process & Software dependency & \texttt{gov2text} inclusion \\
        \hline
        1. Assemble url list. & N/A & no \\
        2. Collect website files. & wget \& Selenium & no \\
        3. Remove names. & \texttt{spacyR} & yes \\
        4. Retain nouns, verbs, adjectives. & \texttt{spacyR} & yes \\
        5. Lemmatize text. & \texttt{spacyR} & yes \\
        6. Retain tokens that are English words. & \texttt{Hunspell} & yes \\
        7. Convert corpus to document-term matrix. & [MARKUS?] & no \\
        \hline
    \end{tabular}
