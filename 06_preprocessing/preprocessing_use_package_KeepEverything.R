...
load("../04_extractText/out/results_parsed_all_KeepEverything.rdata")
...
for(i in 1:length(all_cities)){
  ...
  citydir <- paste0("out/preprocessing_parsed_KeepEverything/", all_cities[i])
...
f <- list.files(path = "out/preprocessing_parsed_KeepEverything", pattern = "parsed_nouns.rdata", full.names = T, recursive = T)
all_nouns <- list()
for(i in 1:length(f)){
  load(f[i])
  all_nouns[[i]] <- nouns
...
save(all_nouns, file = "out/all_nouns_KeepEverything.rdata")
...
f <- list.files(path = "out/preprocessing_parsed_KeepEverything", pattern = "parsed.rdata", full.names = T, recursive = T)
for(i in 1:length(f)){
  load(f[i])
...
f <- list.files(path = "out/preprocessing_parsed_KeepEverything", pattern = "preprocessed.rdata", full.names = T, recursive = T)
tks_city_l <- list()
for(i in 1:length(f)){
  load(f[i])
...
rm(d, d_reordered)
save(tks, file = "out/preprocessed_KeepEverything.rdata")
...
save(d_stm, file = "out/stm_corpus_KeepEverything.rdata")
